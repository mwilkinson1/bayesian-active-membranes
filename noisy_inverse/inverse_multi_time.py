...
# expansion_type = "max" or "mixed" or "uni"
if len(sys.argv) != 5:
    print("Usage: python invsere_multi_time.py <noise_name> <expansion_type> <samples_to_view> <param_num>")
    print("Example: python invsere_multi_time.py s1 max 10 0")
    sys.exit(1)
...
param_num = int(sys.argv[4])

# Validate noise type and param_num
...
if noise_name not in valid_types:
    ...
    sys.exit(1)
if param_num not in [0, 1, 2]:
    print("Invalid param_num. Must be 0, 1, or 2")
    sys.exit(1)
...
# Import data
labels = pd.read_csv(os.path.join(data_dir, f"{noise_name}_{expansion_type}_multi_labels.csv"), header=None, index_col=0)
xx_targets = pd.read_csv(os.path.join(data_dir, f"{noise_name}_xx_{expansion_type}_multi.csv"), header=None, index_col=0)
yy_targets = pd.read_csv(os.path.join(data_dir, f"{noise_name}_yy_{expansion_type}_multi.csv"), header=None, index_col=0)
...
# Calculate offset based on param_num
offset = param_num * 10
...
def multiComponentModel(alpha_1=None, alpha_2=None, xx_observations=None, yy_observations=None):
    ...
    k1 = pyro.sample("k1", dist.Uniform(low=0.00117, high=0.0225))
    k2 = pyro.sample("k2", dist.Uniform(low=8.31, high=15.32))
    kappa = pyro.sample("kappa", dist.Uniform(low=0.27, high=0.3))
    theta = pyro.sample("theta", dist.Uniform(low=0, high=90))
...
# Apply offset to the starting index
# Calculate the end index (last sample we want to include)
end_idx = -(offset + 1)  # -1 because we want to include the last sample
# Calculate the start index (first sample we want to include)
start_idx = end_idx - samples_to_view + 1  # +1 because we want to include the start_idx sample

if end_idx == -1:
    a11 = labels_tensor[start_idx:, 5]
    a22 = labels_tensor[start_idx:, 6]

    xx_obs = xx_targets_tensor[start_idx:, :]
    yy_obs = yy_targets_tensor[start_idx:, :]
else:
    a11 = labels_tensor[start_idx:end_idx + 1, 5]
    a22 = labels_tensor[start_idx:end_idx + 1, 6]

    xx_obs = xx_targets_tensor[start_idx:end_idx + 1, :]
    yy_obs = yy_targets_tensor[start_idx:end_idx + 1, :]
...
samples_pd.to_csv(f"MCMC{noise_name}_Multi_Time_{expansion_type}_{samples_to_view}.csv")
