...
if len(sys.argv) != 3:
    print("Usage: python invsere_single_max.py <data_type> <index>")
    print("Example: python invsere_single_max.py s1 0")
    sys.exit(1)
...
# Import data
labels = pd.read_csv(os.path.join(data_dir, f"{data_type}_max_expansion_labels.csv"), header=None, index_col=0)
xx_targets = pd.read_csv(os.path.join(data_dir, f"{data_type}_max_expansion_xx.csv"), header=None, index_col=0)
yy_targets = pd.read_csv(os.path.join(data_dir, f"{data_type}_max_expansion_yy.csv"), header=None, index_col=0)
...
samples_pd.to_csv(f"MCMC{ind}_{data_type}_Comp_Max.csv")